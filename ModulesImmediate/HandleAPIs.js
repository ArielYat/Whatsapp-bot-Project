...
class HAPI {
    ...
    static async Translate(client, bodyText, chatID, messageID, groupsDict) {
        ...
        if (lang != null) {
            const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=${lang}&dt=t&q=${textToTranslate}`
            request.get(url, function (error, response, body) {
                if (!error && response.statusCode === 200) {
                    const arrayWord = body.match(/"(.)+[[]]]/)[0].replace("[[]]", "").split(",");
                    const translateWord = arrayWord[0].trim();
                    client.reply(chatID, HL.getGroupLang(groupsDict, chatID, "translate_reply") + translateWord, messageID);
                }
            }, null);
        } else client.reply(chatID, HL.getGroupLang(groupsDict, chatID, "translate_language_error"), messageID);
    }
}
