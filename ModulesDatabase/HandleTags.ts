...
export default class HT {
    static async checkTags(client, bodyText, chatID, messageID, authorID, quotedMsgID, groupsDict, usersDict, afkPersons) {
        ...
        for (const tag in tags) {
            if (!bodyText.includes(tag)) continue;
        ...
        if (counter === 0) {
            await client.reply(chatID, await HL.getGroupLang(groupsDict, chatID, "tag_person_doesnt_exist_error"), messageID);
            return;
        }
        try {
    ...
    static async remTag(client, bodyText, chatID, messageID, groupsDict) {
        ...
        if (!groupsDict[chatID].tags) {
            client.reply(chatID, await HL.getGroupLang(groupsDict, chatID, "group_doesnt_have_tags_error"), messageID);
            return;
        }
        if (groupsDict[chatID].doesTagExist(tag)) {
    ...
    static async tagEveryone(client, bodyText, chatID, quotedMsgID, groupsDict) {
        ...
        for (const person of groupsDict[chatID].personsIn) {
            if (!person.afk) stringForSending += "@" + person.personID.replace("@c.us", "") + "\n";
        }
    ...
    static async showTags(client, chatID, messageID, groupsDict) {
        ...
        if (!Object.keys(group.tags).length) {
            await client.reply(chatID, await HL.getGroupLang(groupsDict, chatID, "group_doesnt_have_tags_error"), messageID);
            return;
        }
    ...
    static async addTaggingGroup(client, bodyText, chatID, messageID, groupsDict) {
        ...
        if (!nameArray) {
            client.reply(chatID, await HL.getGroupLang(groupsDict, chatID, "add_tagging_group_no_people_error"), messageID);
            return;
        }

        let phoneNumbersArray = nameArray.map(currentName => {
        ...
        if (phoneNumbersArray.length <= 0) {
            client.reply(chatID, await HL.getGroupLang(groupsDict, chatID, "add_tagging_group_invalid_people_error"), messageID);
            return;
        }
    ...
    static async createTagList(client, bodyText, chatID, groupsDict) {
        if (groupsDict[chatID].tagStack.length !== 0) return;
