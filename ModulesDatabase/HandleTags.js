    ...
    static async checkTags(client, bodyText, chatID, messageID, authorID, quotedMsgID, groupsDict, usersDict) {
        ...
        for (const tag in tags) {
            if (bodyText.includes(tag)) {
                ...
                if ((index <= 0 || !((new RegExp(`(?![ולמבשה])(\\w|${allLanguagesRegex})`)).test(bodyText[index - 1]))) &&
                    (index + tag.length >= bodyText.length || !(new RegExp(`(\w|${allLanguagesRegex})`).test(bodyText[index + tag.length])))) {
                    counter += 1;
